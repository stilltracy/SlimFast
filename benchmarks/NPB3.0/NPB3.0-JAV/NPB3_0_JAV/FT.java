...
public class FT extends FTBase{
  ...
  public static void main(String argv[]){
    ...
    ft.shutdownWorkers();
     ...
     public static void shutdownWorkers(FTBase[] workers)
  {
  ...
  {
	shutdownWorkers(doEvolve);
	shutdownWorkers(doFFT);
  }
  ...
  public void doEvolve(int it){ 
    ...
    int m;  
    for(m=0;m<num_threads;m++)
      synchronized(doEvolve[m].wtf){
        doEvolve[m].done=false;
	doEvolve[m].kt=it;
        doEvolve[m].wtf.notify();
      }
    for(m=0;m<num_threads;m++)
