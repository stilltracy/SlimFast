...
/*This tests that keys are in sequence: sorting of last ranked key seq
  occurs here, but is an untimed operation			       */
     ...
     for(int i=0;i<rankthreads.length;i++)
	{
		synchronized(rankthreads[i].wtf)
		{
			rankthreads[i].shouldDie=true;
			rankthreads[i].wtf.notifyAll();
		}
		//rankthreads[i].stop();
		//rankthreads[i].wtf.notifyAll();
	}		  
...
public Object wtf=new Object();
  void doSort(){
    ...
    for(m=0;m<num_threads;m++)
      synchronized(rankthreads[m].wtf){
        rankthreads[m].done=false;
        rankthreads[m].wtf.notify();
      }
      for(m=0;m<num_threads;m++)
