...
public class LU extends LUBase{
  ...
  public static void shutdownWorkers(LUBase[] workers)
  {
  ...
  {
	shutdownWorkers(scaler);
	shutdownWorkers(upperjac);
	shutdownWorkers(lowerjac);
	shutdownWorkers(adder);
...
//---------------------------------------------------------------------
           if (timeron) timer.start(t_jacld);
             ...
             synchronized(this.wtf){
          for(m=0;m<num_threads;m++)
	     synchronized(lowerjac[m].wtf){
               lowerjac[m].done=false;
               lowerjac[m].wtf.notify();
	       notifyAll();
             }
...
//---------------------------------------------------------------------
  	   if (timeron)  timer.start(t_jacu);
           synchronized(this.wtf){
           for(m=0;m<num_threads;m++)
	     synchronized(upperjac[m].wtf){
               upperjac[m].done=false;
               upperjac[m].wtf.notify();
 	       notifyAll();
            }
