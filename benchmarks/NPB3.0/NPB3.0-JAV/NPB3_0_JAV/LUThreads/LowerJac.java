	...
	try{
      ...
      for(k=1;k<=nz-2;k++){
	    ...
	    try{
	  	if(shouldDie)
			return;
		wtf.wait();}catch(Exception e){}
            synchronized(master.wtf){master.wtf.notify();}
	  }
	}
        step2(k);
 	todo--;
        if(id<num_threads-1)synchronized(neighbor[id+1].wtf){
	  neighbor[id+1].todo++;
	  neighbor[id+1].wtf.notify();
	}
      }
      done=true;
      if(id==num_threads-1) synchronized(master.wtf){master.wtf.notify();}
      else synchronized(neighbor[id+1].wtf){neighbor[id+1].wtf.notify();}
      }
    }
  }
