...
public class SP extends SPBase{
  ...
  public static void main(String argv[]){
   ...
   public static void shutdownWorkers(SPBase[] workers)
  {
	for(int i=0;i<workers.length;i++)
	{
		synchronized(workers[i].wtf)
		{
			workers[i].shouldDie=true;
			workers[i].wtf.notify();
		}
	}
  }	
  public void shutdownWorkers()
  {
	shutdownWorkers(txinverse);
	shutdownWorkers(xsolver);
	shutdownWorkers(ysolver);
	shutdownWorkers(zsolver);
	shutdownWorkers(rhsadder);
	shutdownWorkers(rhscomputer);
  }
...
//---------------------------------------------------------------------
  ...
  public void adi(){ 
	...
	synchronized(txinverse[m].wtf){
          txinverse[m].done=false;
          txinverse[m].wtf.notify();
        }
      for(int m=0;m<num_threads;m++)
        while(!txinverse[m].done){
          try{wtf.wait();}catch(InterruptedException e){} 
          notifyAll();
        }
    }    
    if (timeron)timer.stop(t_txinvr);    
    ...
    synchronized(this.wtf){
      for(int m=0;m<num_threads;m++)
	synchronized(rhsadder[m].wtf){
          rhsadder[m].done=false;
          rhsadder[m].wtf.notify();
        }
      for(int m=0;m<num_threads;m++)
  ...
  synchronized(wtf)
	...
	synchronized(rhscomputer[m].wtf){
          rhscomputer[m].done=false;
          rhscomputer[m].wtf.notify();
        }
    for(m=0;m<num_threads;m++)
  ...
  synchronized(wtf)
    {  int m;
    for(m=0;m<num_threads;m++)
	synchronized(xsolver[m].wtf){
          xsolver[m].done=false;
          xsolver[m].wtf.notify();
        }
    for(m=0;m<num_threads;m++)
  ...
  void doYsolve(){
    ...
    int m;
    for(m=0;m<num_threads;m++)
	synchronized(ysolver[m].wtf){
          ysolver[m].done=false;
          ysolver[m].wtf.notify();
        }
    for(m=0;m<num_threads;m++)
  ...
  void doZsolve(){
    ...
    int m;
    for(m=0;m<num_threads;m++)
	synchronized(zsolver[m].wtf){
          zsolver[m].done=false;
          zsolver[m].wtf.notify();
        }
    for(m=0;m<num_threads;m++)
