...
public class SortedLSNTreeWalker {
    ...
    private FetchResult fetchLSN(
        ...
        try {
            ...
            if (residentNode != null) {
                /*
                 * Concurrent activity (e.g., log cleaning) must have loaded
                 * the node. Just return it and continue.
                 */
                ...
                /*
                 * The resident node is a delta. If it's full LSN is not the
                 * one we intend to fetch below, then queue that node for
                 * fetching later.
                 */
                BIN delta = (BIN) residentNode;
                if (delta.getLastFullLsn() != lsn) {
                    final long fullLsn = delta.getLastFullLsn();
                    pendingLSNs.add(fullLsn);
                    addToLsnINMap(fullLsn, in, index, delta, in.getLsn(index));
                    return null;
                }
