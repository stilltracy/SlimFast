...
public class LN extends Node implements VersionedWriteLoggable {
    ...
    private void init(byte[] data, int off, int len, int doff, int dlen) {
        if (len == 0) {
            ...
            ArrayCopy.copy(data, off, this.data, doff, dlen);
        }
    }
    ...
    public void setEntry(DatabaseEntry entry) {
        ...
        ArrayCopy.copy(data, 0, bytes, 0, len);
        entry.setData(bytes);
    }
    ...
    public static void setEntry(DatabaseEntry dest, byte[] bytes) {
        ...
        if (bytes != null) {
            ...
            if (len == 0) {
                ...
                ArrayCopy.copy(bytes, off, newdata, 0, len);
            }
    ...
    public static void setEntry(DatabaseEntry dest, DatabaseEntry src) {
        ...
        if (src.getData() != null) {
            ...
            if (len == 0) {
                ...
                newdata = new byte[len];
                ArrayCopy.copy(srcBytes, off, newdata, 0, len);
            }
            dest.setData(newdata);
            dest.setOffset(0);
            dest.setSize(len);
    ...
    public static byte[] copyEntryData(DatabaseEntry entry) {
        ...
        ArrayCopy.copy(entry.getData(), entry.getOffset(),
                         newData, 0, len);
        return newData;
    }
    ...
    public static byte[] resolvePartialEntry(DatabaseEntry entry,
        ...
        if (slicelen > 0) {
            ArrayCopy.copy(foundDataBytes, 0, newData, pos, slicelen);
        }
        pos += doff;

        /* Copy in the new data. */
        slicelen = entry.getSize();
        ArrayCopy.copy(entry.getData(), entry.getOffset(), newData, pos,
                         slicelen);
        pos += slicelen;
        ...
        if (slicelen > 0) {
            ArrayCopy.copy(foundDataBytes, doff + dlen, newData, pos,
                             slicelen);
        }
