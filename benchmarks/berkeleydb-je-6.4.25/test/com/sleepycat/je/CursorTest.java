...
public class CursorTest extends DualTestCase {
    ...
    private void insertMultiDb(int numDbs)
        ...
        for (int i = 0; i < numDbs; i++) {
             ...
             * positive.
             */
            DatabaseImpl dbImpl = DbInternal.getDatabaseImpl(myDb[i]);
    ...
    private void checkSearch(Cursor cursor,
        ...
        final OperationStatus status = DbInternal.search(
            cursor, key, null, data, searchMode, null);
    ...
    private void checkSearchBoth(Cursor cursor,
        ...
        final OperationStatus status = DbInternal.searchBoth(
            cursor, key, pKey, data, searchMode, null);
    ...
    public void testInsertionDuringGetNextBinDuringRangeSearch() {
         ...
         */
        DbInternal.getDatabaseImpl(db).getTree().setGetParentINHook(
            new TestHook() {
                @Override
                public void doHook() {
                    /* Only process the first call to the hook. */
                    DbInternal.getDatabaseImpl(db).getTree().
        ...
        Assert.assertSame(OperationStatus.SUCCESS, status);
        Assert.assertEquals(1000, IntegerBinding.entryToInt(key));
    }
