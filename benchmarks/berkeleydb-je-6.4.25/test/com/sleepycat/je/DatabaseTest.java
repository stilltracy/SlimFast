...
public class DatabaseTest extends DualTestCase {
    ...
    */
     ...
     *    3,157,213 env overhead
     *
     * For main cache only:
     * 14,558,904  Internal nodes only
     * 29,575,544  Internal nodes and leaf nodes
     *
     * With off-heap cache:
     *  3,342,787  Internal nodes only: MAIN cache
     *  8,876,994  Internal nodes only: OFF-HEAP cache
     *  3,342,787  Internal nodes and leaf nodes: MAIN cache
     * 21,713,376  Internal nodes and leaf nodes: OFF-HEAP cache
     */
    private void doPreloadAllInCache(final boolean useOffHeapCache)
        ...
        final DatabaseEntry key = new DatabaseEntry(new byte[100]);
        ...
        for (int i = 0; i < nRecs; i += 1) {
            final byte[] keyValue = TestUtils.getTestArray(i);
            System.arraycopy(keyValue, 0, key.getData(), 0, keyValue.length);
        ...
        if (prevBin != DbInternal.getCursorImpl(cursor).getBIN()) {
            nBins += 1;
        }
        cursor.close();

        /* Re-open env with full cache sizes. */
        db.close();
        close(env);
        env = null;
...
//        System.out.println(stats);
        Pair<Long, Long> lnsAndBins = countLNsAndBINs(db);
        assertEquals(
            nBins, lnsAndBins.second() + stats.getOffHeapCachedBINs());
        assertEquals(
            nRecs, lnsAndBins.first() + stats.getOffHeapCachedLNs());
...
//        System.out.println(stats);
        ...
        stats = env.getStats(null);
//        System.out.println(stats);
        lnsAndBins = countLNsAndBINs(db);
        assertEquals(
            nBins, lnsAndBins.second() + stats.getOffHeapCachedBINs());
        assertEquals(
            0, lnsAndBins.first() + stats.getOffHeapCachedLNs());

        db.close();
        close(env);
    }

    private Pair<Long, Long> countLNsAndBINs(final Database db) {
        ...
        for (final IN in : dbImpl.getEnv().getInMemoryINs()) {
            ...
            if (in.getDatabase() != dbImpl || !in.isBIN()) {
                continue;
            }
    ...
    public void testPreloadNoLNs()
        ...
        final boolean embeddedLNs =
            (DbInternal.getEnvironmentImpl(env).getMaxEmbeddedLN() >= 1);
