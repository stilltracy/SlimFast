...
public class DiskOrderedScanTest extends TestBase {
    ...
    public void testBlockedProducer(
        ...
        if (embeddedLNs) {
            assertEquals(queueSize - freeSlots + 300, cnt);
            assertEquals(4, hook.getHookValue().intValue());
        } else {
            assertEquals(0, hook.getHookValue().intValue());
        ...
        if (embeddedLNs) {
            if (keysonly) {
                ...
                assertEquals(6, hook.getHookValue().intValue());
            }
        }
        ...
        if (embeddedLNs) {
            if (keysonly) {
                ...
                assertEquals(7, hook.getHookValue().intValue());
            }
        }
    ...
    public void doTestDeltas(
        ...
        /*
         * Create a non-sticky cursoe so that we can have a stable CursorImpl
         * to use below.
         */
        ...
        if (ohCache != null) {
            final DatabaseImpl dbImpl = DbInternal.getDatabaseImpl(dbs[0]);
        ...
        if (useOffHeapCache) {
            ...
            if (doCkpt) {
                ...
                if (allowEviction) {
                    assertEquals((embeddedLNs ? 11 : 13), nIter);
                } else {
                    assertEquals((embeddedLNs ? 8 : 8), nIter);
                }
            }
        }
