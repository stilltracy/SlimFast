...
public class MasterTransferTest extends RepTestBase {
    ...
    public void testApplicationActivityDuringConversion() throws Exception {
         ...
         */
        List<Transaction> testTxns = new ArrayList<Transaction>();
        ...
        try {
            ...
            Set<CheckedHook> hooks = new HashSet<CheckedHook>();
            ...
            hooks.addAll(setupRNConvertHooks
                         (RepInternal.getRepImpl(masterEnv).getRepNode(),
                          testTxns.get(9),
                          testTxns.get(10),
                          testTxns.get(11),
                          db,
    ...
    public void testTxnThreadInterrupt() throws Exception {
        ...
        awaitInvalidateAndSettle(master, replica);
        try {
            db.close();
    ...
    /**
     * Wait for the former master to become invalidated due to an exception,
     * and the new master to get into the "MASTER" state.
     */
    static void awaitInvalidateAndSettle
        (final RepEnvInfo master, // former master
         final RepEnvInfo replica) // upcoming master
        throws Exception {

        RepTestUtils.awaitCondition(new Callable<Boolean>() {
          @Override
          public Boolean call() {
              return State.MASTER.equals(replica.getEnv().getState()) &&
                  !master.getEnv().isValid();
          }}, 10000);
    }
    ...
    static private Set<Class<? extends Throwable> >
        makeSet(Class<? extends Throwable> clazz) {
    ...
    static private Set<Class<? extends Throwable> >
        makeSet(Class<? extends Throwable> clazz1,
                Class<? extends Throwable> clazz2) {
