...
public class SSLChannelTest extends TestBase {
    ...
    class Packet {
        ...
        public Packet(int dir, int packetSize, int startIdx) {
            this.direction = dir;
    ...
    public void testProtosAndCiphers()
        ...
        props.setProperty(SSL_CIPHER_SUITES,
                          "Foo,SSL_RSA_WITH_RC4_128_SHA,Bar");
        ...
        props = createSSLFactoryProps();
        props.setProperty(SSL_CIPHER_SUITES, "SSL_RSA_WITH_RC4_128_SHA");
    ...
    public void runScenario(Random random,
        ...
        for (int iter = 0; iter < nIters; iter++) {
            ...
            for (int pkt = 0; pkt < numPackets; pkt++) {
                ...
                packets[pkt] = new Packet(dir, packetSize, packetIdx);
        ...
        void processPackets(
            ...
            try {
                for (pkt = 0; pkt < packets.length; pkt++) {
                    ...
                    byte[] packetData = new byte[packet.getPacketSize()];

                    ByteBuffer buffer =
                        ByteBuffer.allocate(packet.getPacketSize());
                    ...
                    if (packet.toServer() != asServer) {
                        ...
                        for (int i = 0; i < packetData.length; i++) {
                            packetData[i] = (byte)(packet.getStartIdx() + i);
                        }
