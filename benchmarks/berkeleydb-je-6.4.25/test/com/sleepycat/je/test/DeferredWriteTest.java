...
public class DeferredWriteTest extends TestBase {
    ...
    private void closeEnv(boolean normalClose,
        ...
        if (env != null) {
            ...
            DbInternal.getEnvironmentImpl(env).stopDaemons();
    ...
    private Database createDb(boolean deferredWrite)
        ...
        assertEquals
            (deferredWrite,
             DbInternal.getDatabaseImpl(db).isDurableDeferredWrite());
        assertEquals
            (deferredWrite,
             DbInternal.getDatabaseImpl(db).isDeferredWriteMode());
        assertEquals
            (false,
             DbInternal.getDatabaseImpl(db).isTemporary());
    ...
    public void testTempIsDeferredWriteMode()
        ...
        long origEndOfLog = DbInternal.getEnvironmentImpl(env)
                                      .getFileManager()
                                      .getNextLsn();

        insertAndCheck(db);

        long endOfLog = DbInternal.getEnvironmentImpl(env)
                                  .getFileManager()
                                  .getNextLsn();
    ...
    public void testTempEvictionAndObsoleteCounting()
        ...
        assertTrue(DbInternal.getEnvironmentImpl(env).
    ...
    private void doCleaning(String minUtilization, String logFileSize)
        ...
        checkExactContentMatch(db, expectedBatch2);
        env.checkpoint(CHECKPOINT_FORCE_CONFIG);
        Trace.trace(DbInternal.getEnvironmentImpl(env), "before clean");
        batchClean();

        Trace.trace(DbInternal.getEnvironmentImpl(env), "after clean");
