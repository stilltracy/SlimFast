...
public class LockManagerTest extends DualTestCase {
    ...
    public void testSR15926LargeNodeIds()
        ...
        final LockManager lockManager = DbInternal.getEnvironmentImpl(env).
    ...
    /**
     * When a true deadlock occurs, check that we have detected it and added
     * information about it to the exception message.
     *
     * The deadlock is:
     *
     *  + T1 locks L1 then L2
     *  + T2 locks L2 then L1
     *
     * T1 waits forever for L2, while T2 does not wait forever for L1, so the
     * deadlock is detected in T2 due to the timeout.
     */
    @Test
    public void testDeadlockMessage()
        throws Throwable {
        ...
        JUnitThread tester1 =
            new JUnitThread("testDeadlockMessage1") {
                public void testBody() throws Throwable {
                    try {
                        /* Lock L1, should always be granted */
                        lockManager.lock(
                            1L, txn1, LockType.WRITE, 1,
                            false, false, null);

                        assertTrue(
                            lockManager.isOwner(1L, txn1, LockType.WRITE));

                        /* Wait for T2 to lock L2. */
                         ...
                         * T2 aborts and releases L2.
                         */
                        lockManager.lock(
                            2L, txn1, LockType.WRITE, 0,
                            false, false, null);
                    } finally {
                        lockManager.release(1L, txn1);
                        lockManager.release(2L, txn1);
        ...
        JUnitThread tester2 =
            new JUnitThread("testDeadlockMessage2") {
                public void testBody() throws Throwable {
                    try {
                        /* Lock L1, should always be granted */
                        lockManager.lock(
                            2L, txn2, LockType.WRITE, 0,
                            false, false, null);

                        assertTrue(
                            lockManager.isOwner(2L, txn2, LockType.WRITE));

                        /* Wait for T1 to lock L1. */
                        while (lockManager.nOwners(1L) < 1) {
                            Thread.yield();
                        }

                        /* Wait for T1 to wait for L2. */
                        while (lockManager.nWaiters(2L) < 1) {
                            Thread.yield();
                        }

                        /*
                         * Try to lock L1, don't wait forever. LockConflict
                         * occurs with deadlock information.
                         */
                        lockManager.lock(
                            1L, txn2, LockType.WRITE, 25,
                            false, false, null);
                        ...
                        assertTrue(
                            msg,
                            msg.contains("" + txn1 + " waits for "));

                        assertTrue(
                            msg,
                            msg.contains("" + txn2 + " waits for "));

                        assertTrue(
                            msg,
                            msg.contains("" + txn2 + " owns "));
                    } finally {
                        lockManager.release(2L, txn2);
                        txn2.removeLock(2L);
                    }
                }
            };
